...
The preceding figure shows three common guest use cases that Cisco ISE provides: Hotspot access, Self-Registered Guest Access, and Full-Sponsored Access.

Hotspot access is a quick and easy guest access that requires no credentials for users to gain internet access.

Self-Registered Access provides a simple self-registration portal that allows guests or contractors to gain appropriate access levels. This portal can also admit registered users with elevated levels of access to certain parts of the corporate network.

Full-Sponsored Access is a role-based access that requires an employee to act as a sponsor to provide guests or contractors with internet and appropriate levels of corporate intranet access.

The Cisco ISE Guest service allows guests, visitors, contractors, consultants, or customers to perform an HTTP or HTTPS login to access a network. The solution supports users with various computer and mobile device browsers and therefore Cisco ISE can identify the device type and provide guests with an easy-to-use onboarding screen. This easy onboarding process often allows guests to connect without the need for specialized assistance, which reduces the burden on IT staff.
